...
impl LoginSlideView {
    ...
    fn privacy_disclaimer_prefix(&self) -> String {
        if self.ai_enabled {
            crate::tr!("auth", "opt-out-analytics-and-ai")
        } else {
            crate::tr!("auth", "opt-out-analytics")
        }
    }
    ...
    fn render_select_auth_content(&self, appearance: &Appearance) -> Vec<Box<dyn Element>> {
        ...
        let tos_line = Flex::row()
            ...
            .with_child(
                ui_builder
                    .link(
                        crate::tr!("auth", "terms-of-service").into(),
                        Some(TOS_URL.into()),
                        None,
    ...
    fn render_browser_open_content(
        ...
        let hint = Flex::column()
            .with_child(
                Flex::row()
                    ...
                    .with_child(
                        ui_builder
                            .link(
                                crate::tr!("auth", "copy-the-url").into(),
                                None,
                                Some(Box::new(|ctx| {
            ...
            .with_child(
                ui_builder
                    .span(crate::tr!("auth", "the-page-manually"))
                    .with_style(sub_text_styles)
                    .build()
                    .finish(),
            )
        ...
        let auth_token: Box<dyn Element> = if self.show_auth_token_input {
            ...
            Flex::row()
                .with_child(
                    ui_builder
                        .link(
                            crate::tr!("auth", "click-here-paste-token-slide").into(),
                            None,
                            Some(Box::new(|ctx| {
