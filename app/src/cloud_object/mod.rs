...
impl CloudObjectMetadataExt for CloudObjectMetadata {
    fn semantic_editing_history(&self, app: &AppContext) -> Option<String> {
        ...
        let full_string = match (editor_string, time_ago_string) {
            (Some(name), Some(time_ago)) if name.is_empty() => crate::tr!("cloud_object", "edited-time-ago", time_ago = time_ago),
            (Some(name), Some(time_ago)) => crate::tr!("cloud_object", "name-edited-time-ago", name = name, time_ago = time_ago),
            (None, Some(time_ago)) => crate::tr!("cloud_object", "edited-time-ago", time_ago = time_ago),
            (Some(name), None) => crate::tr!("cloud_object", "last-edited-by", name = name),
            _ => return None,
        };
    ...
    fn semantic_permadeletion_countdown(&self, app: &AppContext) -> Option<String> {
            ...
            .or_else(|| get_top_folder_trashed_ts(self.folder_id, app))
        {
            ...
            let full_string = match days_left {
                0 | 1 => crate::tr!("cloud_object", "one-day-until-permanent-deletion"),
                _ => crate::tr!("cloud_object", "days-until-permanent-deletion", days_left = days_left),
            };
            Some(full_string)
        } else {
            None
...
impl Space {
    pub fn name(&self, app: &AppContext) -> String {
        match self {
            Space::Personal => crate::tr!("cloud_object", "space-personal"),
            Space::Team { team_uid, .. } => {
                let user_workspaces = UserWorkspaces::as_ref(app);
                if let Some(team) = user_workspaces.team_from_uid(*team_uid) {
                    team.name.clone()
                } else {
                    crate::tr!("cloud_object", "space-team")
                }
            }
            Space::Shared => crate::tr!("cloud_object", "space-shared"),
        }
    }
}
