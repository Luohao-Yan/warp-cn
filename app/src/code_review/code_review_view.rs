    ...
    .with_tooltip(move || {
        ui_builder
            .tool_tip(if is_sidebar_expanded {
                crate::tr!("code_review", "hide-file-navigation")
            } else {
                crate::tr!("code_editor", "review-show-file-navigation")
            })
            .build()
            .finish()
    })
...
pub fn get_discard_button_disabled_tooltip(git_operation_blocked: bool) -> String {
    if git_operation_blocked {
        crate::tr!("code_review", "cannot-discard-while-git-operation")
    } else {
        crate::tr!("code_review", "no-changes-to-discard")
    }
}
...
impl DiscardOperationType {
    pub fn title(&self) -> String {
        match self {
            DiscardOperationType::AllUncommittedChanges => {
                crate::tr!("code_review", "discard-uncommitted-changes-title")
            }
            DiscardOperationType::FileUncommittedChanges => {
                crate::tr!("code_review", "discard-file-uncommitted-changes-title")
            }
            DiscardOperationType::AllChangesAgainstBranch(_) => crate::tr!("code_review", "discard-all-changes-title"),
            DiscardOperationType::FileChangesAgainstBranch(_) => {
                crate::tr!("code_review", "discard-file-all-changes-title")
            }
        }
    }
    ...
    pub fn description(&self) -> Option<String> {
        match self {
            DiscardOperationType::AllUncommittedChanges => Some(crate::tr!("code_review", "discard-uncommitted-desc")),
            DiscardOperationType::FileUncommittedChanges => Some(crate::tr!("code_review", "discard-file-uncommitted-desc")),
            DiscardOperationType::AllChangesAgainstBranch(None) => Some(crate::tr!("code_review", "discard-all-changes-no-branch-desc")),
            DiscardOperationType::FileChangesAgainstBranch(None) => Some(crate::tr!("code_review", "discard-file-no-branch-desc")),
            DiscardOperationType::AllChangesAgainstBranch(Some(_)) => Some(crate::tr!("code_review", "discard-all-changes-no-branch-desc")),
            DiscardOperationType::FileChangesAgainstBranch(Some(branch)) => Some(crate::tr!("code_review", "discard-file-branch-desc", branch = branch.clone())),
        }
    }
...
pub struct CodeReviewView {
    ...
    // Header-specific dropdown menu (crate::tr!("code_editor", "code-review-add-diff-set-context") / crate::tr!("code_editor", "code-comment-add"))
...
impl CodeReviewView {
    ...
    pub fn new(
        ...
        let maximize_button = ctx.add_typed_action_view(move |_| {
            ...
            let (icon, tooltip_text) = (Icon::Maximize, crate::tr!("workspace", "maximize"));
        ...
        let git_primary_action_button = ctx.add_typed_action_view(|_ctx| {
            ActionButton::new(crate::tr!("code_editor", "review-commit"), SecondaryTheme)
        ...
        let undo_action_button = ctx.add_typed_action_view(move |ctx| {
            ...
            let mut action_button = ActionButton::new(crate::tr!("common", "undo-label"), NakedTheme)
                .with_size(ButtonSize::Small)
                .on_click(move |ctx| {
        ...
        let discard_confirm_button = ctx.add_typed_action_view(|_ctx| {
            ActionButton::new(crate::tr!("code_editor", "review-discard-changes"), DangerPrimaryTheme)
        ...
        let discard_cancel_button = ctx.add_typed_action_view(|_ctx| {
            ActionButton::new(crate::tr!("terminal", "tooltip-cancel"), NakedTheme).on_click(|ctx| {
        ...
        let init_project_button = ctx.add_typed_action_view(|_ctx| {
            ActionButton::new(crate::tr!("code_editor", "review-initialize-codebase"), NakedTheme)
                .with_size(ButtonSize::Small)
                .with_tooltip(crate::tr!("code_editor", "review-initialize-codebase-tooltip"))
        ...
        let open_repository_button = ctx.add_typed_action_view(|_ctx| {
            ActionButton::new(crate::tr!("workspace", "open-repository"), NakedTheme)
                .with_size(ButtonSize::Small)
    ...
    fn update_maximize_button(&mut self, ctx: &mut ViewContext<Self>) {
        ...
        let (icon, tooltip) = if is_maximized {
            (Icon::Minimize, crate::tr!("code_editor", "restore"))
        } else {
    ...
    pub(crate) fn build_diff_targets(&self, ctx: &ViewContext<Self>) -> Vec<DiffTarget> {
        ...
        // 1. Always add crate::tr!("code_review", "uncommitted-changes") first.
        targets.push(DiffTarget::new(
            crate::tr!("code_review", "uncommitted-changes"),
            DiffMode::Head,
            matches!(current_mode, DiffMode::Head),
        ));

        // 2. If the current mode targets a branch not in the local branch
        // list, add it after crate::tr!("code_review", "uncommitted-changes").
    ...
    fn build_view_state_for_file_diffs(
        ...
        let discard_tooltip_text = if git_operation_blocked {
            ...
            crate::tr!("code_editor", "review-discard-changes")
        };
        ...
        for file in files {
            ...
            let open_in_tab_button = ctx.add_typed_action_view(move |_ctx| {
                ActionButton::new("", NakedTheme)
                    ...
                    .with_size(ButtonSize::InlineActionHeader)
                    .with_tooltip(crate::tr!("terminal", "open-file"))
                    .on_click(move |ctx| {
            ...
            let add_context_button = ctx.add_typed_action_view(move |_ctx| {
                ActionButton::new("", NakedTheme)
                    ...
                    .with_size(ButtonSize::InlineActionHeader)
                    .with_tooltip(crate::tr!("code_review", "add-file-diff-context"))
                    .on_click(move |ctx| {
            ...
            let copy_path_button = ctx.add_typed_action_view(move |_ctx| {
                ActionButton::new("", NakedTheme)
                    .with_icon(Icon::Copy)
                    .with_size(ButtonSize::InlineActionHeader)
                    .with_tooltip(crate::tr!("code_review", "copy-file-path"))
                    .on_click(move |ctx| {
    ...
    fn render_no_repo_for_env(
        ...
        match self.session_env(app) {
            Some(GitSessionState {
                ...
                // No crate::tr!("workspace", "open-repository") CTA when the session is remote — the
    ...
    fn render_placeholder_header(appearance: &Appearance) -> Box<dyn Element> {
        ...
        let header_text = crate::tr!("code_review", "loading-open-changes");
    ...
    fn render_error_state(&self, error: &str, appearance: &Appearance) -> Box<dyn Element> {
        ...
        let main_column = Flex::column()
            ...
            .with_child(
                Text::new(
                    crate::tr!("code_review", "error-loading-diffs"),
                    appearance.ui_font_family(),
    ...
    pub fn render_no_repo_found_state(
        ...
        let main_column = Flex::column()
            ...
            .with_child(
                Text::new(
                    crate::tr!("code_review", "cannot-detect-diffs"),
                    appearance.ui_font_family(),
                    appearance.ui_font_size() + 2.,
                )
                .with_style(Properties::default().weight(Weight::Semibold))
    ...
    fn render_no_changes_state(
        ...
        let mut zero_state_column = Flex::column()
            ...
            .with_child(
                Container::new(
                    Text::new(
                        crate::tr!("code_review", "track-changes-hint"),
                        appearance.ui_font_family(),
                        14.,
                    )
        ...
        match result {
            ReviewSubmissionResult::Success {
                ...
                ToastStack::handle(ctx).update(ctx, |stack, ctx| {
                    let toast = DismissibleToast::default(crate::tr!("code_review", "comments-sent-to-agent").into());
            ...
            ReviewSubmissionResult::Error => {
                ...
                let error_message = crate::tr!("code_review", "could-not-submit-comments");
    ...
    fn render_file_content(&self, file: &FileState, appearance: &Appearance) -> Box<dyn Element> {
        ...
        if file.file_diff.is_binary {
            Self::styled_file_content_container(
                Text::new(
                    crate::tr!("code_review", "binary-file"),
                    appearance.monospace_font_family(),
                    appearance.monospace_font_size(),
                )
                .with_color(theme.main_text_color(theme.background()).into())
            ...
            Self::styled_file_content_container(
                Text::new(
                    crate::tr!("code_review", "file-renamed-without-changes"),
                    appearance.monospace_font_family(),
                    appearance.monospace_font_size(),
                )
                .with_color(theme.main_text_color(theme.background()).into())
            ...
            Self::styled_file_content_container(
                Text::new(
                    crate::tr!("code_review", "new-empty-file"),
                    appearance.monospace_font_family(),
                    appearance.monospace_font_size(),
                )
                .with_color(theme.main_text_color(theme.background()).into())
            ...
            Self::styled_file_content_container(
                Text::new(
                    crate::tr!("code_review", "unable-to-load-file"),
                    appearance.ui_font_family(),
                    appearance.ui_font_size(),
                )
                .with_color(theme.sub_text_color(theme.background()).into())
    ...
    fn render_file_row(&self, app: &AppContext) -> Box<dyn Element> {
        ...
        if self.discard_dialog_state.discard_file_paths.is_empty() {
            return Text::new(
                crate::tr!("code_review", "no-file-selected"),
                appearance.ui_font_family(),
                appearance.ui_font_size(),
            )
            .with_color(
        ...
        let CodeReviewViewState::Loaded(loaded) = self.state() else {
            return Text::new(
                crate::tr!("code_review", "no-files-to-discard"),
                appearance.ui_font_family(),
                appearance.ui_font_size(),
            )
            .with_color(
            ...
            CodeEditorEvent::DiffReverted => {
                ...
                crate::workspace::ToastStack::handle(ctx).update(ctx, |toast_stack, ctx| {
                    let toast = crate::view_components::DismissibleToast::default(
                        crate::tr!("code_review", "diff-removed"),
                    )
    ...
    fn insert_selection_as_context(
        ...
        if let Some(terminal_view) = self.attach_target_terminal(ctx) {
            ...
            if is_long_running {
                ...
                crate::workspace::ToastStack::handle(ctx).update(ctx, |toast_stack, ctx| {
                    let toast = crate::view_components::DismissibleToast::default(
                        crate::tr!("code_review", "cannot-attach-context-terminal"),
                    )
                    .with_object_id(toast_id);
                    toast_stack.add_ephemeral_toast(toast, self.window_id, ctx);
                });
            ...
            if !is_input_box_visible {
                ...
                ToastStack::handle(ctx).update(ctx, |toast_stack, ctx| {
                    let toast = DismissibleToast::default(
                        crate::tr!("code_review", "cannot-attach-diff-no-input"),
                    )
                    .with_object_id(toast_id);
                    toast_stack.add_ephemeral_toast(toast, self.window_id, ctx);
                });
    ...
    /// make the button stay on crate::tr!("code_editor", "review-commit") after a successful commit.
    ...
    fn open_git_dialog(&mut self, kind: GitDialogKind, ctx: &mut ViewContext<Self>) {
        ...
        let dialog = match kind {
            GitDialogKind::Commit => {
                // Hide the crate::tr!("code_editor", "review-commit-and-create-pr") intent when it wouldn't make
        ...
        match mode {
            PrimaryGitActionMode::Commit => {
                ...
                self.git_primary_action_button.update(ctx, |button, ctx| {
                    button.set_label(crate::tr!("code_editor", "review-commit"), ctx);
                ...
                self.git_operations_chevron.update(ctx, |button, ctx| {
                    button.set_disabled(disabled, ctx);
                    button.set_tooltip(disabled.then_some(crate::tr!("code_review", "no-git-actions")), ctx);
                });
            }
            PrimaryGitActionMode::Push => {
                self.git_primary_action_button.update(ctx, |button, ctx| {
    ...
    /// Returns the crate::tr!("code_editor", "review-commit") dropdown item. Label/icon/action are fixed;
    ...
    fn commit_menu_item(disabled: bool) -> MenuItem<CodeReviewAction> {
        MenuItemFields::new(crate::tr!("code_editor", "review-commit"))
            .with_icon(Icon::GitCommit)
    ...
    fn push_or_publish_menu_item(has_upstream: bool, disabled: bool) -> MenuItem<CodeReviewAction> {
        if has_upstream {
            MenuItemFields::new(crate::tr!("code_editor", "code-review-push"))
                .with_icon(Icon::ArrowUp)
                .with_on_select_action(CodeReviewAction::OpenPushDialog)
                .with_disabled(disabled)
                .into_item()
        } else {
            MenuItemFields::new(crate::tr!("code_editor", "code-review-publish"))
    ...
    /// crate::tr!("code_editor", "code-review-create-pr") to open the dialog. Create PR is disabled on main, when the
    ...
    fn pr_menu_item(&self, app: &AppContext) -> MenuItem<CodeReviewAction> {
        ...
        if let Some(pr_info) = self.pr_info(app) {
            ...
            let has_upstream = diff_state.upstream_ref(app).is_some();
            let upstream_differs_from_main = diff_state.upstream_differs_from_main(app);
            MenuItemFields::new(crate::tr!("code_editor", "code-review-create-pr"))
                .with_icon(Icon::Github)
    ...
    fn header_menu_items_legacy(
        ...
        if FeatureFlag::DiffSetAsContext.is_enabled() && has_changes {
            items.push(
                MenuItemFields::new(crate::tr!("code_editor", "code-review-add-diff-set-context"))
                    .with_icon(Icon::Paperclip)
        ...
        let (comment_label, comment_icon) = if self.get_existing_diffset_comment(ctx).is_some() {
            (crate::tr!("code", "show-saved-comment"), Icon::MessageText)
        } else {
            (crate::tr!("code_editor", "code-comment-add"), Icon::MessagePlusSquare)
        };
    ...
    fn header_menu_items_new(
        ...
        if FeatureFlag::DiscardPerFileAndAllChanges.is_enabled() && has_changes {
            items.push(
                MenuItemFields::new(crate::tr!("code_editor", "review-discard-all"))
                    .with_icon(Icon::ReverseLeft)
...
impl BackingView for CodeReviewView {
    ...
    fn render_header_content(
        ...
        view::HeaderContent::simple(crate::tr!("code_review", "reviewing-code-changes"))
    }
