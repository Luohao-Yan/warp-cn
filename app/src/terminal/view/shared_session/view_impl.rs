...
impl TerminalView {
    ...
    fn start_cloud_followup_from_tombstone(
        ...
        let Some(ambient_agent_view_model) = self.ambient_agent_view_model.as_ref() else {
            self.show_error_toast(crate::tr!("terminal", "couldnt-continue-cloud-task"), ctx);
            return;
        };
    ...
    fn end_session_on_inactivity_period_expired(&mut self, ctx: &mut ViewContext<Self>) {
        ...
        self.show_persistent_toast(
            crate::tr!("terminal", "sharing-ended-inactivity"),
            ToastFlavor::Error,
            ctx,
        );
    }
    ...
    fn revoke_roles_on_inactivity_period_expired(&mut self, ctx: &mut ViewContext<Self>) {
        ...
        if num_executors > 0 {
            ...
            self.show_persistent_toast(
                crate::tr!("terminal", "sharing-edit-revoked-inactivity"),
                ToastFlavor::Error,
                ctx,
            );
        }
    ...
    pub fn maybe_show_role_changed_toast(
        ...
        {
            self.show_persistent_toast(
                crate::tr!("terminal", "editing-revoked-sharer-idle"),
                ToastFlavor::Error,
                ctx,
            );
        }
    ...
    pub fn session_sharing_context_menu_items(
        ...
        if !model.shared_session_status().is_sharer_or_viewer() {
            let share_session_label = crate::tr!("terminal", "menu-share-session-ellipsis");
            items.push(
            ...
            let stop_sharing_label = crate::tr!("terminal", "menu-stop-sharing");
            items.push(
        ...
        if model.shared_session_status().is_sharer_or_viewer() {
            let copy_session_link_label = crate::tr!("terminal", "menu-copy-session-sharing-link");
            items.push(
    ...
    pub fn render_input_request_edit_access_button(
        ...
        appearance
            ...
            .with_text_label(crate::tr!("terminal", "menu-request-edit-access").into())
            .build()
