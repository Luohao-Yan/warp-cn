use std::sync::Arc;use pathfinder_geometry::vector::vec2f;
...
static_tr!(SEARCH_PLACEHOLDER_TEXT, "agent_cloud", "search-models");
static_tr!(BUTTON_TOOLTIP, "agent_cloud", "choose-agent-model");
static_tr!(NO_RESULTS_LABEL, "agent_cloud", "no-results");
static_tr!(DEFAULT_MODEL_LABEL, "agent_cloud", "default-model");
    ...
    fn refresh_button(&mut self, ctx: &mut ViewContext<Self>) {
        ...
        let active_label = match self.active_harness(ctx) {
            Some(harness) if !matches!(harness, Harness::Oz | Harness::Unknown) => self
                ...
                .unwrap_or_else(|| DEFAULT_MODEL_LABEL.get().to_owned()),
    ...
    fn refresh_menu(&mut self, ctx: &mut ViewContext<Self>) {
        ...
        if items.is_empty() {
            ...
            items.push(MenuItem::Item(
                MenuItemFields::new(NO_RESULTS_LABEL.get())
