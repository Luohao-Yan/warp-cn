...
impl TeamsPageView {
    ...
    pub fn new(ctx: &mut ViewContext<TeamsPageView>) -> Self {
        ...
        let transfer_ownership_modal = ctx.add_typed_action_view(|ctx| {
            Modal::new(
                Some(crate::tr!("settings", "transfer-ownership-confirm")),
        ...
        match event {
            ...
            UserWorkspacesEvent::JoinTeamWithTeamDiscoverySuccess => {
                ...
                let message = self
                    ...
                    .as_ref(ctx)
                    .current_team()
                    .map_or(crate::tr!("settings", "joined-team-success"), |team| {
                        crate::tr!("settings", "joined-team-success", team_name = team.name.as_str())
                    });
...
impl TeamsWidget {
    ...
    fn render_team_member_cost_info(
        ...
        let prorated_message = if has_admin_permissions {
            crate::tr!("settings", "admin-charged-prorated")
        } else {
            crate::tr!("settings", "non-admin-charged-prorated")
        };
        ...
        {
            crate::tr!("settings", "additional-members-cost-with-price", monthly_cost = format!("{:.2}", monthly_cost), yearly_cost = format!("{:.2}", yearly_cost), prorated_message = prorated_message.as_str())
        } else {
            crate::tr!("settings", "additional-members-cost-no-price", prorated_message = prorated_message.as_str())
        };
    ...
    fn render_discoverability_toggle_section(
        ...
        let team_discoverability_instructions =
            crate::tr!("settings", "allow-discover-domain", domain = domain);
        section.add_child(
            Container::new(self.render_sub_text(
    ...
    fn render_create_team_page(
        ...
        page.add_child(render_sub_header(appearance, crate::tr!("settings", "teams"), None));
        page.add_child(
            self.render_sub_header_with_subtext_color(appearance, crate::tr!("settings", "create-a-team")),
        );
        page.add_child(
            Container::new(
