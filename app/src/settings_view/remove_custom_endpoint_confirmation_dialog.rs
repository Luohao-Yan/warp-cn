...
static_tr!(REMOVE_ENDPOINT_TITLE, "code", "remove-endpoint-title");
static_tr!(REMOVE_ENDPOINT_DESC, "code", "remove-endpoint-desc");
static_tr!(REMOVE_ENDPOINT_BUTTON, "code", "remove-endpoint-button");
...
impl RemoveCustomEndpointConfirmationDialog {
    pub fn new(ctx: &mut ViewContext<Self>) -> Self {
        ...
        let confirm_button = ctx.add_typed_action_view(|_| {
            ActionButton::new(REMOVE_ENDPOINT_BUTTON.get(), DangerPrimaryTheme).on_click(|ctx| {
...
impl View for RemoveCustomEndpointConfirmationDialog {
    ...
    fn render(&self, app: &AppContext) -> Box<dyn Element> {
        ...
        let description = REMOVE_ENDPOINT_DESC.get().to_string();
        ...
        let dialog = Dialog::new(
            REMOVE_ENDPOINT_TITLE.get().to_string(),
            Some(description),
