...
static KEYBINDING_LABEL: LazyLock<String> = LazyLock::new(|| crate::tr!("settings", "keybinding-label"));
static CONTEXT_LABEL: LazyLock<String> = LazyLock::new(|| crate::tr!("settings", "context-label"));
...
impl KeybindingRow {
    ...
    fn render_summary(
        ...
        let element = render_columns(
            render_text(
                binding.description.in_context(DescriptionContext::Default).to_string(),
                None,
                appearance,
            ),
            keystroke,
    ...
    fn render(
        ...
        let local_only_icon_state = if *CloudPreferencesSettings::as_ref(app).settings_sync_enabled
        {
            Some(LocalOnlyIconState::Visible {
                ...
                custom_tooltip: Some(crate::tr!("settings", "keyboard-shortcuts-not-synced")),
            })
        } else {
            None
        };
