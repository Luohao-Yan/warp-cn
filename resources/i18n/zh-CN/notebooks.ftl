...
notebooks-notebooks-command-from = 来自 { $location_name } 的命令
notebooks-notebooks-could-not-read = 无法读取 { $name }
notebooks-notebooks-loading-file = 正在加载 { $name }…
notebooks-notebooks-editor-is-editing = { $editor } 正在编辑
notebooks-notebooks-pasting-a11y = 正在粘贴：{ $content }
notebooks-notebooks-copy-link-a11y = { notebooks-copy-link-a11y }
notebooks-notebooks-open-link-a11y = 打开链接：{ $url }
notebooks-notebooks-secondary-click-a11y = 右键点击 { $url }
notebooks-notebooks-modifier-click-a11y = [{ $modifier } 点击]
notebooks-notebooks-insert-block-a11y = 插入 { $block } 块
notebooks-notebooks-change-lang-a11y = 将代码块语言更改为 { $language }
