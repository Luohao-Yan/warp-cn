...
code-code-showing-references = Showing { $count } references
code-code-server-status-message = { $name }: { $message }
code-code-server-stopped = { $name }: stopped
code-code-server-error = { $name }: error
...
code-code-enable-server = Enable { $server }
code-code-install-server = Install { $server }
code-code-install-servers = { code-install-servers }
code-code-enable-servers = { code-enable-servers }
code-code-installing-server = Installing { $server }…
code-code-lang-support-unavailable-file = { code-lang-support-unavailable-file }
code-code-lang-support-not-enabled = Language support is not currently enabled for { $name }
code-code-this-codebase = { code-this-codebase }
code-code-lang-server-unavailable = { code-lang-server-unavailable }
code-code-lang-support-unavailable = Language support is unavailable for { $name }
