...
ai-assistant-ai-perm-desc-agent-decides = The Agent chooses the safest path: acting on its own when confident, and asking for approval when uncertain.
ai-assistant-ai-perm-desc-always-allow = Give the Agent full autonomy — no manual approval ever required.
ai-assistant-ai-perm-desc-always-ask = Require explicit approval before the Agent takes any action.
ai-assistant-ai-perm-desc-ask-on-first-write = The agent will ask for permission the first time it needs to interact with a running command. After that, it will continue automatically for the rest of that command.
ai-assistant-ai-perm-desc-never = Computer use tools are disabled and will not be available to the Agent.
ai-assistant-ai-perm-desc-never-ask = The Agent cannot run child agents and the run_agents tool will not be available.
ai-assistant-ai-perm-desc-ask-unless-auto = The Agent may ask a question and pause for your response, but will continue automatically when auto-approve is on.
ai-assistant-ai-perm-desc-never-ask-questions = The Agent will not ask questions and will continue with its best judgment.
ai-assistant-ai-perm-desc-always-ask-questions = The Agent may ask a question and will pause for your response even when auto-approve is on.
